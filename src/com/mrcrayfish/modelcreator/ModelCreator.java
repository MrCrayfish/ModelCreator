...
public class ModelCreator extends JFrame
{
	...
	private ElementManager sidebarManager;
	...
	public ModelCreator(String title)
	{
		...
		{
			initWindow();
			GLDisplayUtils.initDisplay(canvas);
	...
	public void initComponents()
	{
		setupMenuBar();
		canvas = initCanvas();
		sidebarManager = new SidebarPanel(this);
		uvSidebar = new UVSidebar("UV Editor", sidebarManager);
		initScrollPane(sidebarManager);
	}
	...
	private void initScrollPane(ElementManager sidebarManager)
	{
		JScrollPane scroll = new JScrollPane((JPanel) sidebarManager);
	...
	private void loop() throws LWJGLException
	{
		...
		while (!Display.isCloseRequested() && !getCloseRequested())
		{
			...
			GLDisplayUtils.handleInput( offset, width, height, camera, getHeight(), lastMouseX, lastMouseX, grabbing, grabbed, sidebarManager, activeSidebar
			);
			...
			GLDisplayUtils.draw(camera);
			GLDisplayUtils.drawPerspective(sidebarManager);

			GLDisplayUtils.glDisableProp();
			GLU.gluOrtho2D(0, width, height, 0);

			glMatrixMode(GL_MODELVIEW);
			glLoadIdentity();

			GLDisplayUtils.drawOverlay(offset,  width, height, camera, activeSidebar, getHeight() );
	...
	protected static class GLDisplayUtils {
		private int lastMouseX, lastMouseY;

		protected static void initDisplay(Canvas canvas) {
			try {
		...
		public static void drawPerspective(ElementManager sidebarManager) {
			glClearColor(0.92F, 0.92F, 0.93F, 1.0F);
			drawGrid();

			glTranslatef(-8, 0, -8);
			for (int i = 0; i < sidebarManager.getElementCount(); i++) {
				GL11.glLoadName(i + 1);
				Element cube = sidebarManager.getElement(i);
				cube.draw();
				GL11.glLoadName(0);
				cube.drawExtras(sidebarManager);
			}
		...
		public static int select(int x, int y, int width, int height, Camera camera) {
		...
		public static void drawOverlay(int offset, int width, int height, Camera camera, Sidebar activeSidebar, int outerHeight) {
			glPushMatrix();
			{
			...
			if (activeSidebar != null)
				activeSidebar.draw(offset, width, height, outerHeight);
		...
		public static void handleInput(int offset, int width, int height, Camera camera, int outerHeight, int lastMouseX, int lastMouseY, Boolean grabbing, Element grabbedElement, ElementManager sidebarManager, Sidebar activeSidebar
		) {
			...
			if (Mouse.getX() < offset) {
				activeSidebar.handleInput(outerHeight);
			} else {

				if (Keyboard.isKeyDown(Keyboard.KEY_LCONTROL))
					if (grabbedElement == null) {
						if (Mouse.isButtonDown(0) | Mouse.isButtonDown(1)) {
							int sel = GLDisplayUtils.select(Mouse.getX(), Mouse.getY(), width, height, camera);
							if (sel >= 0) {
								grabbedElement = sidebarManager.getAllElements().get(sel);
								sidebarManager.setSelectedElement(sel);
							}
						}
					} else {
						Element element = grabbedElement;
						int state = GLDisplayUtils.getCameraState(camera);
					...
					if (Mouse.isButtonDown(0)) {
						...
						final float modifier = GLDisplayUtils.applyLimit(cameraMod * 0.1f);
		...
		private static void glDisableProp() {
